if [ $# -eq 0 ]
  then
    echo "./submarine-installer UNUSED_PARTITION USERNAME MASTER_IP PRIVATE_IP1 PRIVATE_IP2 PRIVATE_IP3 NODE_ID"
    exit 1
fi
...
sudo chown -R $USERNAME:timeseries-PG0 /mnt
...
cd /users/$USERNAME/submarine-stack-setup
cd conf
...
cd /users/$USERNAME/submarine-stack-setup/other-dependencies-installer 
