...
public class ServiceClient extends AppAdminClient implements SliderExitCodes,
  ...
  public int actionFlexThemis(String serviceName, Map<String, String>
  componentCountStrings, Map<String, String>
  componentTfConfigStrings, Map<String, List<String>> componentLocalityPrefs) throws YarnException, IOException {
Map<String, Long> componentCounts =
    new HashMap<>(componentCountStrings.size());
Service persistedService =
    ServiceApiUtil.loadService(fs, serviceName);
...
flexComponentsThemis(serviceName, componentCounts, persistedService, componentTfConfigStrings, componentLocalityPrefs);
return EXIT_SUCCESS;
}  
  ...
  private Map<String, Long> flexComponentsThemis(String serviceName,
	      Map<String, Long> componentCounts, Service persistedService, Map<String, String>
  componentTfConfigStrings, Map<String, List<String>> componentLocalityPrefs)
	      throws YarnException, IOException {
	    ServiceApiUtil.validateNameFormat(serviceName, getConfig());
	    ...
	    for (Component persistedComp : persistedService.getComponents()) {
	      ...
	      if (componentCounts.containsKey(persistedComp.getName())) {
	        ...
	        if(componentTfConfigStrings == null) {
		        // build the request
		        countBuilder.setName(persistedComp.getName())
		            .setNumberOfContainers(persistedComp.getNumberOfContainers());
	        } else {
		        // build the request
		        countBuilder.setName(persistedComp.getName())
		            .setNumberOfContainers(persistedComp.getNumberOfContainers()).setTfConfig(componentTfConfigStrings.get(name))
		            .addAllLocalityPrefs(componentLocalityPrefs.get(name));
		        LOG.info("[THEMIS] - Setting the appropriate TF_CONFIG!");
		        LOG.info("[THEMIS] - LOCAL_PREFS = {} and {}", componentLocalityPrefs.get(name), countBuilder.getLocalityPrefsList());
	        }
