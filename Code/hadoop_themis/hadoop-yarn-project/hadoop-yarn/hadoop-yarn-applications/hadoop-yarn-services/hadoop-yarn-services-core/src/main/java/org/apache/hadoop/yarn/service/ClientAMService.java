  ...
  @Override public FlexComponentsResponseProto flexComponents(
    ...
    if (!request.getComponentsList().isEmpty()) {
      for (ComponentCountProto component : request.getComponentsList()) {
    	...
    	if (!component.hasTfConfig()) {  
	        event = new ComponentEvent(component.getName(), FLEX)
	                  .setDesired(component.getNumberOfContainers());
    	} else {
    		LOG.info("[THEMIS] Flex request contains TF_CONFIG = {}", component.getTfConfig());
    		LOG.info("[THEMIS] Flex request contains LOCAL_PREFS = {}", component.getLocalityPrefsList());
	        event = new ComponentEvent(component.getName(), FLEX)
	            .setDesired(component.getNumberOfContainers())
	            .setTfConfig(component.getTfConfig())
	            .setLocalityPrefs(component.getLocalityPrefsList());
    	}
