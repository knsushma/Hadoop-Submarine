...
public class RunJobParameters extends RunParameters {
  ...
  private List<String> psLocalityPrefs = new ArrayList<String>();
  private List<String> masterLocalityPrefs = new ArrayList<String>();
  private List<String> workerLocalityPrefs = new ArrayList<String>();
  ...
  public void updateParametersByParsedCommandline(CommandLine parsedCommandLine,
    ...
    if (nWorkers >= 2 && nPS > 0) {
      ...
      String psLocalityPrefsStr = parsedCommandLine.getOptionValue(CliConstants.PS_LOCALITY_PREF);
      String masterLocalityPrefsStr = parsedCommandLine.getOptionValue(CliConstants.MASTER_LOCALITY_PREF);
      String workerLocalityPrefsStr = parsedCommandLine.getOptionValue(CliConstants.WORKER_LOCALITY_PREF);
      
      for (String hostname : psLocalityPrefsStr.trim().split(",")) {
          hostname = hostname.trim();
          psLocalityPrefs.add(hostname);
      }
      
      for (String hostname : masterLocalityPrefsStr.trim().split(",")) {
          hostname = hostname.trim();
          masterLocalityPrefs.add(hostname);
      }
      
      for (String hostname : workerLocalityPrefsStr.trim().split(",")) {
          hostname = hostname.trim();
  ...
  public List<String> getPSLocalityPrefs() {
	    return psLocalityPrefs;
	  }
  
  public List<String> getMasterLocalityPrefs() {
	    return masterLocalityPrefs;
	  }
  
  public List<String> getWorkerLocalityPrefs() {
